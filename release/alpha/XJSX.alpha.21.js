...
return 
   ...
   //   time += performance.now() - tm
      ...
      if (this.hasOnboardProcess()) {
        /***check end**/
        this.modules.end.operations[0].callback(void 0,this.onboardProcesses[this.onboardProcesses.length-1].process,this);
      }
  ...
  __core__.createModule([
    {
      ...
      callback: function (e, node,eval) {
       ...
       if (!tmp) {
         console.error("use-template","'template#"+e+"'no such template element found");
       }
...
// _eval=_eval()
   ...
   ({__proto__:__core__,eval:__core__._eval(eval),onboardProcesses:[]})._observer(document);
